...
namespace QMC.LCP_280.Process.Unit.FormMain
{
    public partial class DieOutputControl : UserControl
    {
        ...
        private void SyncBinViewFromRecipe()
        {
            try
            {
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ask.ShowDialog("Question", "Bin 정보를 초기화하시겠습니까?") != DialogResult.Yes)
                {
                    return;
                }
                var outputStage = TryGetUnit<OutputStage>(Equipment.UnitKeys.OutputStage);
                ...
                // 내부에서 wafer/material 정리까지 수행(명시적으로 의미가 드러나게 named arguments 사용)
                outputStage.ResetForNewRun(moveToSafeReady: false, clearWafer: true, clearOffsets: true);

                // UI 초기화(이 컨트롤은 picked history가 없으므로 표시 데이터만 정리)
        ...
        public void SetDieList(List<MaterialDie> dies)
        {
            ...
            var items = _dies.Select(d =>
            {
                ...
                //var modelPos = new PointD(d.MapX, d.MapY);
                var displayPos = ToDisplay(modelPos);

                GetDisplayColors(d, out var fill, out var border);
                return new DisplayView_DieOutput.DisplayItem
                {
                    Position = displayPos,
                    DieMap = new PointD(d.MapX, d.MapY),
        ...
        /// </summary>
        public void UpdateDie(PointD binCoord, DieProcessState newState)
        {
            var die = _dies.FirstOrDefault(d =>
                Math.Round(d.BinX) == binCoord.X &&
                Math.Round(d.BinY) == binCoord.Y);
        ...
        private DisplayView_DieOutput.ItemState ConvertState(DieProcessState state)
        {
            ...
            switch (state)
            {
                ...
                case DieProcessState.Skip:
                    return DisplayView_DieOutput.ItemState.Skip;
        ...
        private void DisplayView1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                ShowDieTooltip(die, (PointD)e.Location);
        ...
        private void ShowDieTooltip(MaterialDie die, PointD clientPoint)
        {
            try
            {
        ...
        // [ADD] BinNo -> Color 팔레트(원하는대로 수정)
        // 보통 GoodBin BinNo를 기준으로 색을 달리 주는 용도
        private static readonly Dictionary<int, Color> BinNoColors = new Dictionary<int, Color>
        {
            { 1, Color.LimeGreen },
            { 11, Color.LimeGreen },
            ...
            { 31, Color.Gold },
            { 41, Color.Orange },
            { 51, Color.MediumPurple },
            { 61, Color.Cyan },
            { 71, Color.SandyBrown },
            { 81, Color.Chartreuse },
        };

        private static Color? GetBinNoRangeColor(int rank)
        {
            if (rank <= 0) return null;

            if (rank >= 1 && rank <= 10) return BinNoColors[1];
            if (rank >= 11 && rank <= 20) return BinNoColors[11];
            if (rank >= 21 && rank <= 30) return BinNoColors[21];
            if (rank >= 31 && rank <= 40) return BinNoColors[31];
            if (rank >= 41 && rank <= 50) return BinNoColors[41];
            if (rank >= 51 && rank <= 60) return BinNoColors[51];
            if (rank >= 61 && rank <= 70) return BinNoColors[61];
            if (rank >= 71 && rank <= 80) return BinNoColors[71];
            if (rank >= 81 && rank <= 90) return BinNoColors[81];

            // 범위 밖은 미지정(기존 상태색/기존 BinNo 팔레트로 fallback)
            return null;
        }

        // [ADD] BinType/BinNo/Rank 기반으로 표시 색을 결정
        // [CHG] BinType/BinNo/Rank 기반으로 표시 색을 결정
        private static void GetDisplayColors(MaterialDie die, out Color? fill, out Color? border)
        {
            fill = null;
            border = null;
            ...
            // [ADD] Rank!=1 이면 표시에서 제외(빈칸)
            //  - FillColor/BorderColor를 null로 유지
            //  - SetDieList에서 State를 Empty로 내려서 "흰색"으로 표시
            //if (die.Rank != 1)
            //{
            //    return;
            //}
            ...
            // Skip / Fail 등은
            if (die.State == DieProcessState.Skip || die.IsPass == false)
            {
                fill = Color.DimGray;
            ...
            // Rejected / Fail 등은 빨강
            if (die.State == DieProcessState.Rejected || die.IsPass == false)
            {
                fill = Color.Red;
            ...
            if (bin == null)
                return; // bin 없으면 기존 State 색을 사용

            // 1) NG는 항상 빨강(우선순위 최상)
            if (bin.BinType == QMC.Common.PKGTester.BinningType.NgBin)
            {
                fill = Color.Violet;
            ...
            // 2) Good이면 BinNo 팔레트 적용
            if (bin.BinType == QMC.Common.PKGTester.BinningType.GoodBin)
            {
                var BinNoColor = GetBinNoRangeColor(die.Rank);
                if (BinNoColor.HasValue)
                {
                    fill = BinNoColor.Value;
                    border = Color.FromArgb(200, Color.Black);
                    return;
                }

                if (BinNoColors.TryGetValue(bin.BinNo, out var c))
                    fill = c;
                else
