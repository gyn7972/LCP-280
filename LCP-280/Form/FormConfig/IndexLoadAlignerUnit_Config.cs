...
{
    public partial class IndexLoadAlignerUnit_Config : Form
    {
        private const string _UNIT_NAME = "IndexLoadAligner";

        private Equipment _Equipment => Equipment.Instance;
        private IndexLoadAligner _unit;
        private IndexLoadAlignerConfig _cfg;

        private IndexLoadAlignerRecipe TeachingRecipe => _cfg?.TeachingRecipe; // [ADD]
        ...
        public IndexLoadAlignerUnit_Config()
        {
            ...
            this.Shown += IndexLoadAlignerUnit_Config_Shown;
            this.FormClosed += IndexLoadAlignerUnit_Config_FormClosed;
        ...
        private void IndexLoadAlignerUnit_Config_Shown(object sender, EventArgs e)
        {
        ...
        private void IndexLoadAlignerUnit_Config_FormClosed(object sender, FormClosedEventArgs e)
        {
            EquipmentRecipe.CurrentRecipeChanged -= EquipmentRecipe_CurrentRecipeChanged;
        }
        ...
        private void ReloadTeachingFromRecipeAndRefreshUi()
        {
            ...
            // [FIX] PositionTeachingControl 내부가 예전 recipe reference를 들고있을 수 있으므로 재주입
            positionTeachingControl?.SetUnitData(_unit, _cfg, recipe);
            positionTeachingControl?.RefreshPositionList();

            // Config 값도 UI에 다시 반영 (레시피 전환/티칭 로드 후 UI 갱신 누락 방지)
            try
            {
            ...
            try
            {
                ...
                Log.Write(_UNIT_NAME, nameof(ReloadTeachingFromRecipeAndRefreshUi),
                    $"MR.Name='{mr?.Name}', TeachingRecipe.Name='{recipe.Name}', TP.Count={(recipe.TeachingPositions?.Count ?? 0)}");
            }
            catch { }
        }
        ...
        private void InitializeUnit()
        {
            try
            {
                ...
                {
                    _unit = unit as IndexLoadAligner;
                    _cfg = _unit?.Config;
                }
