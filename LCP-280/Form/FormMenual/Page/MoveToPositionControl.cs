...
{
    public partial class MoveToPositionControl : UserControl
    {
        #region 필드 / 구조
        ...
        // ★ [ADD] 축 항목(Title)을 Display 스타일로 표시할지 여부
        [Browsable(true), Category("Teaching"), Description("축명/포지션 행을 Display 스타일(검은배경/초록글씨/큰폰트)로 표시")]
        [DefaultValue(true)]
        public bool UseDisplayStyleForAxisRows { get; set; } = true;

        // ★ [ADD] Display 스타일에 사용할 폰트(기본: PropertyCollectionView 내부 기본값을 쓰지만, 필요시 여기서 덮어쓰기)
        [Browsable(true), Category("Teaching"), Description("Display 스타일 폰트(지정 시 PropertyCollectionView.SetDisplayStyle 호출)")]
        public Font AxisDisplayFont { get; set; } = null;
        ...
        private void MoveSelected()
        {
            try
            {
                ...
                if (!Equipment.EnsureAxisReadyForAutoOrMove("TeachingPositionMove"))
                    return;
        ...
        {
            if (tp == null || positionEditorView == null) 
                return;
            ...
            try
            {
                ...
                // ★ [ADD] Display 스타일로 처리할 Title 리스트를 수집
                ...
                if (addMi != null && tp.AxisPositions != null)
                {
                    foreach (var kv in tp.AxisPositions)
                    {
                        ...
                        // 값 단위 표기(원하면 "mm"로 고정 등 가능)
                        var unit = "mm";

                        addMi.Invoke(pc, new object[] { title, unit, kv.Value });

                        if (UseDisplayStyleForAxisRows)
                        {
                ...
                // PropertyCollectionView.SetProperties(...)
                var setMi = positionEditorView.GetType().GetMethod("SetProperties",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

                setMi?.Invoke(positionEditorView, new object[] { pc });

                // ★ [ADD] 특정 행을 Display 스타일로 전환 (PropertyCollectionView인 경우만)
                var pcv = positionEditorView as QMC.Common.PropertyCollectionView;
                if (pcv != null)
                {
                    if (AxisDisplayFont != null)
                    {
                        // 글꼴만 덮어쓰고 나머지는 기본(검은/초록/센터) 유지
                        pcv.SetDisplayStyle(AxisDisplayFont, Color.Black, Color.Lime, ContentAlignment.MiddleCenter);
                    }

                    if (UseDisplayStyleForAxisRows && displayTitles.Count > 0)
                    {
                        pcv.SetDisplayStyleTitles(displayTitles.ToArray());
                        pcv.SetReadOnlyTitles(displayTitles.ToArray()); // Display는 보통 ReadOnly가 자연스러움
                    }
                }
            }
            catch (Exception ex)
