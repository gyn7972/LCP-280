...
{
    [Serializable]
    public sealed class OutputDieTransferRecipe
    : QMC.Common.BaseRecipe, QMC.LCP_280.Process.Unit.FormConfig.IHasTeachingPositions
    {
        ...
        public OutputDieTransferRecipe() : this(null)
        {

        }
        public OutputDieTransferRecipe(string name = null) : base(name)
        {
        ...
        //    var rc = Load();
        //    Log.Write(nameof(OutputDieTransferRecipe), nameof(LoadAndBindAxes),
        ...
        private const string FilePrefix = "OutputDieTransferRecipe";
        ...
        // [CHG] 최종 경로:
        // /Recipes/{CurrentRecipeName}/OutputDieTransferRecipe{Name}.json
        ...
        // 이전 버전 1) /Recipes/OutputDieTransferRecipe/{CurrentRecipeName}/{Name}.json
        private string GetLegacyFilePath_TypeAndRecipeFolder()
        {
            ...
            var root = AppDomain.CurrentDomain.BaseDirectory;
            var dir = System.IO.Path.Combine(root, "Recipes", typeof(OutputDieTransferRecipe).Name, safeCurrentRecipe);
            return System.IO.Path.Combine(dir, safeName + ".json");
        }

        // 이전 버전 2) /Recipes/OutputDieTransferRecipe/{Name}.json
        private string GetLegacyFilePath_TypeFolderOnly()
        {
            var safeName = SanitizeFilePart(string.IsNullOrWhiteSpace(Name) ? "default" : Name);

            var root = AppDomain.CurrentDomain.BaseDirectory;
            var dir = System.IO.Path.Combine(root, "Recipes", typeof(OutputDieTransferRecipe).Name);
            return System.IO.Path.Combine(dir, safeName + ".json");
        }
        ...
        public int LoadFromCurrentRecipeFolderOrMigrate()
        {
            try
            {
                ...
                var legacyObj = JsonConvert.DeserializeObject<OutputDieTransferRecipe>(json);
                if (legacyObj == null)
                    return -1;
