...
namespace QMC.LCP_280.Process.Component
{
    public sealed class WaferManager
    {
        ...
        public void SetRecipeKeys(string carrierId, int slot, List<string> keys, bool force = false)
        {
            ...
            //if (wafer.Summary.InProcess && !force) 
            if (!force)
                return;
        ...
        /// <summary>
        /// 장비 구동 중 "현재 웨이퍼 공정" 데이터를 취합/관리하는 단일 Summary (SSOT)
        /// - UI/로그/저장은 여기서 스냅샷을 가져다 사용
        /// </summary>
        public WaferSummary CurrentSummary { get; } = new WaferSummary();

        /// <summary>
        /// 웨이퍼 공정 시작(바코드 확정/투입 시작 등) 시 호출
        /// </summary>
        public void BeginCurrentWafer(string waferId, string binId, string machineName)
        {
            CurrentSummary.BeginTotalSummary(waferId, binId, machineName);
        }

        /// <summary>
        /// 웨이퍼 공정 종료(언로딩 완료 등) 시 호출
        /// </summary>
        public void EndCurrentWafer()
        {
            CurrentSummary.EndTotalSummary();
        }

        public WaferSummary.WaferTotalSummaryRow GetCurrentRowSnapshot()
        {
            return CurrentSummary.GetRowSnapshot();
        }
