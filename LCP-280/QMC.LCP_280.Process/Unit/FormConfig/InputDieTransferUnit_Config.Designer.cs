...
{
    partial class InputDieTransferUnit_Config
    {
        ...
        private void InitializeComponent()
        {
            ...
            // 
            // InputDieTransferUnit_Config
            // 
            this.AutoScaleDimensions = new System.Drawing.SizeF(96F, 96F);
            this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
            this.ClientSize = new System.Drawing.Size(1264, 780);
            this.Controls.Add(this.mainTableLayoutPanel);
            this.Name = "InputDieTransferUnit_Config";
            this.Text = "InputDieTransfer Unit Configuration";
        ...
        // ===== Digital IO 초기화 (InputDieTransfer Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                const string UNIT_NAME = "InputDieTransfer";
                if (eq?.Units != null &&
                    eq.Units.TryGetValue(UNIT_NAME, out var unit) &&
                    unit is InputDieTransfer transfer &&
                    transfer.InputDieTransferConfig != null)
                {
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 InputDieTransfer Unit 가져오기
        ...
        private void OnPositionItemSelected(object sender, int selectedIndex)
        {
            try
            {
                ...
                {
                    var transfer = unit as InputDieTransfer;
                    if (transfer != null && selectedIndex >= 0 && selectedIndex < transfer.InputDieTransferConfig.TeachingPositions.Count)
                    {
                        var tp = transfer.InputDieTransferConfig.TeachingPositions[selectedIndex];
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                transfer.InputDieTransferConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });
