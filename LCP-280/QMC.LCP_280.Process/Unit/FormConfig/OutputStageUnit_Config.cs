...
namespace QMC.LCP_280.Process.Unit
{
    ...
    /// </summary>
    public partial class OutputStageUnit_Config : Form
    {
        private const string _UNIT_NAME = "OutputStage";
        private Equipment _Equipment => Equipment.Instance;
        private OutputStage _OutputStage;
        private OutputStageConfig _cfg;
        ...
        private void PopulateAllAxesInJogControl()
        {
            try
            {
                ...
                if (_OutputStage?.Axes == null || _OutputStage.Axes.Count == 0) { jogControl.SetTeachingAxisList(null); return; }
                var axisNames = _OutputStage.Axes.Values.Where(a => a != null).Select(a => a.Name ?? a.Setup?.Name).Where(n => !string.IsNullOrWhiteSpace(n)).Distinct().ToArray();
        ...
        {
            if (!_sizeMismatchWarned && (width != _designerSize.Width || height != _designerSize.Height)) _sizeMismatchWarned = true;
            try { SuspendLayout(); Size = new Size(width, height); Invalidate(); Update(); } finally { ResumeLayout(true); }
        }
        ...
        private void btnCurrentPos_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && _OutputStage?.Axes != null && _OutputStage.Axes.TryGetValue(axisKey, out var direct)) axis = direct;
                    if (axis == null && _OutputStage?.Axes != null)
                        foreach (var p in _OutputStage.Axes) if (p.Value != null && string.Equals(p.Value.Name, axisKey, StringComparison.OrdinalIgnoreCase)) { axis = p.Value; break; }
                ...
                foreach (var ex in tp.ExtraInfo) pc.Add(new StringProperty($"Extra: {ex.Key}", ex.Value?.ToString() ?? ""));
