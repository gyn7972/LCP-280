...
{
    partial class IndexChipProberUnit_Config
    {
        ...
        private void InitializeComponent()
        {
            ...
            // 
            // IndexChipProberUnit_Config
            // 
            this.AutoScaleDimensions = new System.Drawing.SizeF(96F, 96F);
            this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
            this.ClientSize = new System.Drawing.Size(1264, 780);
            this.Controls.Add(this.mainTableLayoutPanel);
            this.Name = "IndexChipProberUnit_Config";
            this.Text = "IndexChipProber Unit Configuration";
        ...
        // ===== Digital IO 초기화 (IndexChipProber Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                const string UNIT_NAME = "IndexChipProber";
                if (eq?.Units != null &&
                    eq.Units.TryGetValue(UNIT_NAME, out var unit) &&
                    unit is IndexChipProber prober &&
                    prober.IndexChipProberConfig != null)
                {
                    var cfg = prober.IndexChipProberConfig;
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 IndexChipProber Unit 가져오기
                var equipment = Equipment.Instance;
                const string UNIT_NAME = "IndexChipProber";

                if (equipment.Units.TryGetValue(UNIT_NAME, out var unit))
                {
                    var prober = unit as IndexChipProber;
                    // TeachingPositions 멤버를 직접 사용하여 Position 이름 리스트 추출
                    if (prober?.TeachingPositions != null && prober.TeachingPositions.Count > 0)
                    {
                        var positionNames = prober.TeachingPositions.Select(tp => tp.Name).ToArray();
        ...
        private void OnPositionItemSelected(object sender, int selectedIndex)
        {
            try
            {
                ...
                {
                    var prober = unit as IndexChipProber;
                    if (prober != null && selectedIndex >= 0 && selectedIndex < prober.IndexChipProberConfig.TeachingPositions.Count)
                    {
                        var tp = prober.IndexChipProberConfig.TeachingPositions[selectedIndex];
        ...
        private void ShowTeachingPositionInPropertyCollectionView(int selectedIndex)
        {
            ...
            {
                var prober = unit as IndexChipProber;
                var config = prober?.IndexChipProberConfig;
        ...
        private void btnMovePosition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= prober.IndexChipProberConfig.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                var tp = prober.IndexChipProberConfig.TeachingPositions[selIndex];
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && prober.Axes.TryGetValue(axisKey, out var directAxis)) axis = directAxis;
                    if (axis == null)
                    {
                        // Name 매칭 시도
                        foreach (var aPair in prober.Axes)
                        {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && prober.Axes.TryGetValue(kv.Key, out var directAxis)) axis = directAxis;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= prober.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                var target = prober.TeachingPositions[selIndex];
                ...
                prober.IndexChipProberConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });

                // 저장 후 재로드 & 재바인딩 (선택적으로 최신 반영)
                prober.IndexChipProberConfig.LoadAndBindAxes(Equipment.Instance.AxisManager);
                prober.TeachingPositions.Clear();
                foreach (var tp in prober.IndexChipProberConfig.TeachingPositions)
                    prober.TeachingPositions.Add(tp);
