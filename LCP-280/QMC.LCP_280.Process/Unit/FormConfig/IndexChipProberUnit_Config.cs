...
namespace QMC.LCP_280.Process.Unit
{
    ...
    /// </summary>
    public partial class IndexChipProberUnit_Config : Form
    {
        private const string _UNIT_NAME = "IndexChipProber";
        private Equipment _Equipment => Equipment.Instance;
        private IndexChipProber _IndexChipProber { get; set; }
        private IndexChipProberConfig _cfg;
        private readonly Size _designerSize;
        private bool _sizeMismatchWarned;

        public IndexChipProberUnit_Config()
        {
            InitializeComponent();
            ...
            Console.WriteLine($"✅ IndexChipProber_Config 생성자 완료");
        }
        ...
        private void InitializeUnit()
        {
            try
            {
                ...
                {
                    _IndexChipProber = unit as IndexChipProber;
                    _cfg = _IndexChipProber.IndexChipProberConfig;
                }

                if (_IndexChipProber == null)
                {
        ...
        private void btnCurrentPos_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    // 2) Unit의 축 사전 키로 찾기
                    if (axis == null && _IndexChipProber.Axes != null)
                    {
                        QMC.Common.Motions.MotionAxis directAxis;
                        if (_IndexChipProber.Axes.TryGetValue(axisKey, out directAxis))
                            axis = directAxis;
                    }
                    ...
                    if (axis == null && _IndexChipProber.Axes != null)
                    {
                        foreach (var pair in _IndexChipProber.Axes)
                        {
                            var a = pair.Value;
