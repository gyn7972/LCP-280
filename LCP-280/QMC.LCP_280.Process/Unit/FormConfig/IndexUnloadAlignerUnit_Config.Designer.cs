...
namespace QMC.LCP_280.Process.Unit
{
    partial class IndexUnloadAlignerUnit_Config
    {
        ...
        // ===== Digital IO 초기화 (IndexUnloadAligner Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                const string UNIT_NAME = "IndexUnloadAligner";
                if (eq?.Units != null &&
                    eq.Units.TryGetValue(UNIT_NAME, out var unit) &&
                    unit is IndexUnloadAligner aligner &&
                    aligner.IndexUnloadAlignerConfig != null)
                {
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 IndexUnloadAligner Unit 가져오기
                var equipment = Equipment.Instance;
                const string UNIT_NAME = "IndexUnloadAligner";

                if (equipment.Units.TryGetValue(UNIT_NAME, out var unit))
                {
                    var aligner = unit as IndexUnloadAligner;
        ...
        private void OnPositionItemSelected(object sender, int selectedIndex)
        {
            try
            {
                ...
                {
                    var aligner = unit as IndexUnloadAligner;
                    if (aligner != null && selectedIndex >= 0 && selectedIndex < aligner.IndexUnloadAlignerConfig.TeachingPositions.Count)
                    {
                        var tp = aligner.IndexUnloadAlignerConfig.TeachingPositions[selectedIndex];
        ...
        private void ShowTeachingPositionInPropertyCollectionView(int selectedIndex)
        {
            ...
            {
                var aligner = unit as IndexUnloadAligner;
                var config = aligner?.IndexUnloadAlignerConfig;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                aligner.IndexUnloadAlignerConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });
