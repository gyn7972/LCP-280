...
namespace QMC.LCP_280.Process.Unit
{
    ...
    public partial class IndexLoadAlignerUnit_Config : Form
    {
        ...
        private IndexLoadAligner _IndexLoadAligner { get; set; }
        private IndexLoadAlignerConfig _cfg; //= controller.IndexLoadAlignerConfig;
        ...
        public IndexLoadAlignerUnit_Config()
        {
            ...
            Console.WriteLine($"✅ IndexLoadAlignerUnit_Config 생성자 완료");
        }
        ...
        private void InitializeUnit()
        {
            try
            {
                ...
                {
                    _IndexLoadAligner = unit as IndexLoadAligner;
                    _cfg = _IndexLoadAligner.IndexLoadAlignerConfig;
                }

                if (_IndexLoadAligner == null)
                {
        ...
        private void btnCurrentPos_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    // 2) Unit의 축 사전 키로 찾기
                    if (axis == null && _IndexLoadAligner.Axes != null)
                    {
                        QMC.Common.Motions.MotionAxis directAxis;
                        if (_IndexLoadAligner.Axes.TryGetValue(axisKey, out directAxis))
                            axis = directAxis;
                    }
                    ...
                    if (axis == null && _IndexLoadAligner.Axes != null)
                    {
                        foreach (var pair in _IndexLoadAligner.Axes)
                        {
                            var a = pair.Value;
