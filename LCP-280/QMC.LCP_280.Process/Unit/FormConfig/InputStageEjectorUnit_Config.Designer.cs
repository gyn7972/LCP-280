...
{
    partial class InputStageEjectorUnit_Config
    {
        ...
        private void InitializeComponent()
        {
            ...
            // 
            // InputStageEjectorUnit_Config
            // 
            this.AutoScaleDimensions = new System.Drawing.SizeF(96F, 96F);
            this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
            this.ClientSize = new System.Drawing.Size(1264, 780);
            this.Controls.Add(this.mainTableLayoutPanel);
            this.Name = "InputStageEjectorUnit_Config";
            this.Text = "InputStageEjector Unit Configuration";
        ...
        // ===== Digital IO 초기화 (InputStageEjector Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                const string UNIT_NAME = "InputStageEjector";
                if (eq?.Units != null &&
                    eq.Units.TryGetValue(UNIT_NAME, out var unit) &&
                    unit is InputStageEjector ejector &&
                    ejector.InputStageEjectorConfig != null)
                {
                    var cfg = ejector.InputStageEjectorConfig;
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 InputStageEjector Unit 가져오기
                var equipment = Equipment.Instance;
                const string UNIT_NAME = "InputStageEjector";

                if (equipment.Units.TryGetValue(UNIT_NAME, out var unit))
                {
                    var ejector = unit as InputStageEjector;
                    // TeachingPositions 멤버를 직접 사용하여 Position 이름 리스트 추출
                    if (ejector?.TeachingPositions != null && ejector.TeachingPositions.Count > 0)
                    {
                        var positionNames = ejector.TeachingPositions.Select(tp => tp.Name).ToArray();
        ...
        private void OnPositionItemSelected(object sender, int selectedIndex)
        {
            try
            {
                ...
                {
                    var ejector = unit as InputStageEjector;
                    if (ejector != null && selectedIndex >= 0 && selectedIndex < ejector.InputStageEjectorConfig.TeachingPositions.Count)
                    {
                        var tp = ejector.InputStageEjectorConfig.TeachingPositions[selectedIndex];
        ...
        private void ShowTeachingPositionInPropertyCollectionView(int selectedIndex)
        {
            ...
            {
                var ejector = unit as InputStageEjector;
                var config = ejector?.InputStageEjectorConfig;
        ...
        private void btnMovePosition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= ejector.InputStageEjectorConfig.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                var tp = ejector.InputStageEjectorConfig.TeachingPositions[selIndex];
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && ejector.Axes.TryGetValue(axisKey, out var directAxis)) axis = directAxis;
                    if (axis == null)
                    {
                        // Name 매칭 시도
                        foreach (var aPair in ejector.Axes)
                        {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && ejector.Axes.TryGetValue(kv.Key, out var directAxis)) axis = directAxis;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= ejector.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                var target = ejector.TeachingPositions[selIndex];
                ...
                ejector.InputStageEjectorConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });

                // 저장 후 재로드 & 재바인딩 (선택적으로 최신 반영)
                ejector.InputStageEjectorConfig.LoadAndBindAxes(Equipment.Instance.AxisManager);
                ejector.TeachingPositions.Clear();
                foreach (var tp in ejector.InputStageEjectorConfig.TeachingPositions)
                    ejector.TeachingPositions.Add(tp);
