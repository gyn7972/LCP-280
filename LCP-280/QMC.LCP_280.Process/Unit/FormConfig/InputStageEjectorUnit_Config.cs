...
{
    /// <summary>
    /// InputStageEjector Unit Config Form (UI 로직 분리)
    /// </summary>
    public partial class InputStageEjectorUnit_Config : Form
    {
        ...
        private InputStageEjector _InputStageEjector;
        ...
        private void InitializeUnit()
        {
            try
            {
                ...
                {
                    _InputStageEjector = unit as InputStageEjector;
                    _cfg = _InputStageEjector?.InputStageEjectorConfig;
                }

                if (_InputStageEjector == null)
                {
                    MessageBox.Show(
        ...
        private void PopulateAllAxesInJogControl()
        {
            try
            {
                ...
                if (_InputStageEjector?.Axes == null || _InputStageEjector.Axes.Count == 0)
                {
                    jogControl.SetTeachingAxisList(null);
                    return;
                }

                var axisNames = _InputStageEjector
                    .Axes
                    .Values
                    .Where(a => a != null)
        ...
        private void SetTeachingPositionItems()
        {
            try
            {
                if (_InputStageEjector?.TeachingPositions != null &&
                    _InputStageEjector.TeachingPositions.Count > 0)
                {
                    var names = _InputStageEjector
                        .TeachingPositions
                        .Select(tp => tp.Name)
                        .ToArray();
        ...
        {
            if (_InputStageEjector?.InputStageEjectorConfig?.TeachingPositions == null)
            {
                return;
            }

            var list = _InputStageEjector.InputStageEjectorConfig.TeachingPositions;
        ...
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                if (_InputStageEjector?.InputStageEjectorConfig != null)
                {
                    var cfg = _InputStageEjector.InputStageEjectorConfig;
        ...
        private void btnMovePosition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= _InputStageEjector.InputStageEjectorConfig.TeachingPositions.Count)
                {
                    MessageBox.Show(
                ...
                var tp = _InputStageEjector.InputStageEjectorConfig.TeachingPositions[selIndex];
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && _InputStageEjector.Axes.TryGetValue(axisKey, out var directAxis))
                    {
                        axis = directAxis;
                    }
                    ...
                    if (axis == null)
                    {
                        foreach (var pair in _InputStageEjector.Axes)
                        {
                            if (pair.Value != null &&
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && _InputStageEjector.Axes.TryGetValue(kv.Key, out var directAxis))
                    {
                        axis = directAxis;
                    }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int selIndex = GetSelectedPositionIndex();
                if (selIndex < 0 || selIndex >= _InputStageEjector.TeachingPositions.Count)
                {
                    MessageBox.Show(
                        "선택된 Teaching Position이 없습니다.",
                        "알림",
                ...
                var target = _InputStageEjector.TeachingPositions[selIndex];
                ...
                _InputStageEjector.TeachingPositions.Clear();
                foreach (var tp in _InputStageEjector.InputStageEjectorConfig.TeachingPositions)
                {
                    _InputStageEjector.TeachingPositions.Add(tp);
                }
        ...
        private void btnCurrentPos_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && _InputStageEjector.Axes != null &&
                        _InputStageEjector.Axes.TryGetValue(axisKey, out var directAxis))
                    {
                        axis = directAxis;
                    }
