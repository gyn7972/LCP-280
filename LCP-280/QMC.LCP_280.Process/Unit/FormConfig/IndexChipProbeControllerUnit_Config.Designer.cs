...
using System.Text.RegularExpressions; // ★ 추가: 키 정규화
...
namespace QMC.LCP_280.Process.Unit
{
    partial class IndexChipProbeControllerUnit_Config
    {
        ...
        // 추가: 디자이너에서 사용하는 컨트롤 필드 선언 누락 보강
        private System.Windows.Forms.TableLayoutPanel positionTableLayoutPanel; // 위치 티칭 레이아웃
        private System.Windows.Forms.GroupBox gbTeachingMove;                  // Teaching Move 그룹
        private IndividualMenuButton btnMovePosition;                          // Move Position 버튼
        private RadioButtonView rbTeachingMoveMode;                            // Fine/Coarse 라디오 버튼 뷰
        private System.Windows.Forms.Panel editorPanel;                        // 에디터 패널
        private PropertyCollectionView positionEditorView;                     // 위치 편집 뷰
        private ListBoxItemsView positionItemView;                             // Position 항목 뷰
        private System.Windows.Forms.TableLayoutPanel ioTableLayoutPanel;      // IO 레이아웃
        private ListBoxItemsView axisPositionsView;                            // 축 포지션 뷰
        private System.Windows.Forms.TableLayoutPanel mainTableLayoutPanel;    // 메인 레이아웃
        private System.Windows.Forms.Panel positionItemPanel;                  // (예비) 포지션 아이템 패널
        // Axis 목록 (추가)
        ...
        /// <summary>
        /// JogControl 에 IndexChipProbeController Unit 의 모든 축을 표시 (Position 선택과 무관)
        /// </summary>
        private void PopulateAllAxesInJogControl()
        {
            try
            {
                ...
                const string UNIT_NAME = "IndexChipProbeController";
                var eq = Equipment.Instance;
                if (eq?.Units == null) return;
                if (!eq.Units.TryGetValue(UNIT_NAME, out var u)) return;
                var controller = u as IndexChipProbeController;
                if (controller?.Axes == null || controller.Axes.Count == 0)
                {
                    jogControl.SetTeachingAxisList(null);
                    return;
                }
                var axisNames = controller.Axes.Values
                    .Where(a => a != null)
        ...
        // ===== Digital IO 초기화 (IndexChipProbeController Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                if (eq?.Units != null &&
                    ...
                    unit is IndexChipProbeController controller &&
                    controller.IndexChipProbeControllerConfig != null)
                {
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 IndexChipProbeController Unit 가져오기
                ...
                if (equipment.Units.TryGetValue(UNIT_NAME, out var unit))
                {
                    ...
                    if (controller?.TeachingPositions != null && controller.TeachingPositions.Count > 0)
                    {
                        var positionNames = controller.TeachingPositions.Select(tp => tp.Name).ToArray();
        ...
        private void ShowTeachingPositionInPropertyCollectionView(int selectedIndex)
        {
            ...
            {
                var controller = unit as IndexChipProbeController;
                var config = controller?.IndexChipProbeControllerConfig;
        ...
        private void btnMovePosition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= controller.IndexChipProbeControllerConfig.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                var tp = controller.IndexChipProbeControllerConfig.TeachingPositions[selIndex];
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && controller.Axes.TryGetValue(axisKey, out var directAxis)) axis = directAxis;
                    if (axis == null)
                    {
                        // Name 매칭 시도
                        foreach (var aPair in controller.Axes)
                        {
                ...
                foreach (var kv in tp.AxisPositions)
                {
                    ...
                    if (axis == null && controller.Axes.TryGetValue(kv.Key, out var directAxis)) axis = directAxis;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= controller.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                var target = controller.TeachingPositions[selIndex];
                ...
                controller.IndexChipProbeControllerConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });

                // 저장 후 재로드 & 재바인딩 (선택적으로 최신 반영)
                controller.IndexChipProbeControllerConfig.LoadAndBindAxes(Equipment.Instance.AxisManager);
                controller.TeachingPositions.Clear();
                foreach (var tp in controller.IndexChipProbeControllerConfig.TeachingPositions)
                    controller.TeachingPositions.Add(tp);
