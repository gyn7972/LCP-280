...
namespace QMC.LCP_280.Process.Unit
{
    partial class InputStageUnit_Config
    {
        ...
        // ===== Digital IO 초기화 (InputStage Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                if (eq?.Units != null &&
                    ...
                    unit is InputStage transfer &&
                    transfer.InputStageConfig != null)
                {
                    var cfg = transfer.InputStageConfig;
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 InputStage Unit 가져오기
        ...
        private void ShowTeachingPositionInPropertyCollectionView(int selectedIndex)
        {
            ...
            {
                var stage = unit as InputStage;
                var config = stage?.InputStageConfig;
        ...
        private void btnMovePosition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= stage.InputStageConfig.TeachingPositions.Count)
                {
                    MessageBox.Show("선택된 Teaching Position이 없습니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                var tp = stage.InputStageConfig.TeachingPositions[selIndex];
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                stage.InputStageConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });
                ...
                foreach (var tp in stage.InputStageConfig.TeachingPositions)
