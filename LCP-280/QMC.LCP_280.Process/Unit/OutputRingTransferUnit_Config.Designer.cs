...
{
    partial class OutputRingTransferUnit_Config
    {
        ...
        private void InitializeComponent()
        {
            ...
            // 
            // OutputRingTransferUnit_Config
            // 
            this.AutoScaleDimensions = new System.Drawing.SizeF(96F, 96F);
            this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Dpi;
            this.ClientSize = new System.Drawing.Size(1264, 780);
            this.Controls.Add(this.mainTableLayoutPanel);
            this.Name = "OutputRingTransferUnit_Config";
            this.Text = "OutputRingTransfer Unit Configuration";
        ...
        // ===== Digital IO 초기화 (OutputRingTransfer Unit 관련 IO 자동 필터) =====
        private void InitializeDigitalIO()
        {
            try
            {
                ...
                const string UNIT_NAME = "OutputRingTransfer";
                if (eq?.Units != null &&
                    eq.Units.TryGetValue(UNIT_NAME, out var unit) &&
                    unit is OutputRingTransfer transfer &&
                    transfer.OutputRingTransferConfig != null)
                {
        ...
        private void SetAxisDefinitionsToAxisListBox()
        {
            try
            {
                // Equipment에서 OutputRingTransfer Unit 가져오기
        ...
        private void OnPositionItemSelected(object sender, int selectedIndex)
        {
            try
            {
                ...
                {
                    var transfer = unit as OutputRingTransfer;
                    if (transfer != null && selectedIndex >= 0 && selectedIndex < transfer.OutputRingTransferConfig.TeachingPositions.Count)
                    {
                        var tp = transfer.OutputRingTransferConfig.TeachingPositions[selectedIndex];
        ...
        private void btnMovePosition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selIndex < 0 || selIndex >= transfer.OutputRingTransferConfig.TeachingPositions.Count)
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                transfer.OutputRingTransferConfig.SetTeachingPosition(new TeachingPosition(target.Name, new Dictionary<string, double>(target.AxisPositions), target.Description) { ExtraInfo = new Dictionary<string, object>(target.ExtraInfo) });
