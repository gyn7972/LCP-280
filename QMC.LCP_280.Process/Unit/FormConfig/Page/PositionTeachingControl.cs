...
namespace QMC.LCP_280.Process.Unit.FormConfig
{
    public partial class PositionTeachingControl : UserControl
    {
        ...
        // (정석) 신규 유닛은 recipe.TeachingPositions가 단일 소스
        private IndexChipProbeControllerRecipe _recipe;
        ...
        {
            SetUnitData(unit, config, null);
        }

        // 정석(권장): IndexChipProbeControllerRecipe를 직접 받음
        public void SetUnitData(BaseUnit unit, BaseConfig config, IndexChipProbeControllerRecipe recipe)
        {
            _unit = unit;
            _config = config;
            _recipe = recipe;
        ...
        {
            if (_recipe?.TeachingPositions != null)
                return _recipe.TeachingPositions;
