...
namespace QMC.LCP_280.Process.Unit
{
    public sealed class IndexChipProber : BaseUnit<IndexChipProberConfig>, IDisposable
    {
        ...
        //    {
        //        var res = tester?.Result;
        //        var ranges = tester?.GetCurrentBinRanges(); // 새 헬퍼 사용
        //        var set = tester?.ConditionSet;
        //        if (res == null || set == null) return -1;
        ...
        //        string waferID = wafer?.WaferId ?? "UNKNOWN_WAFER";
        //        string logFile = System.IO.Path.Combine(logDir, $"Result_{waferID}_{DateTime.Now:yyyyMMdd}.csv");
        //        string noFile = logFile + ".no";

        //        // 행 번호 사이드카
        //        int lastNo = 0;
        //        if (System.IO.File.Exists(noFile))
        //            int.TryParse(System.IO.File.ReadAllText(noFile).Trim(), out lastNo);
        //        int nextRowNo = lastNo + 1;

        //        var bin = res.BinningResult;
        //        int binNo = bin?.BinNo ?? -1;
        //        string binLabel = string.IsNullOrWhiteSpace(bin?.BinLabel) ? "NG" : bin.BinLabel;

        //        string nameCell = GetRecipeNameFallback(waferID, binLabel);

        //        var items = set.Items?.ToList() ?? new List<TestConditionItem>();
        ...
        //        string GetRange(string itemName)
        //        {
        //            if (string.IsNullOrWhiteSpace(itemName) || ranges == null) 
        //                return "- ~ -";


        //            if (!ranges.ContainsKey(itemName)) return "- ~ -";
        //            var r = ranges[itemName];
        //            if (r == null || r.Ignore) return "- ~ -";
        //            return $"{r.Min}~{r.Max}";
        //        }

        //        string GetMeasured(string itemName)
        //        {
        //            if (string.IsNullOrWhiteSpace(itemName)) return "-";
        //            if (!res.Items.TryGetValue(itemName, out var r) || r == null) return "-";
        //            double v = r.Value;
        //            if (double.IsNaN(v)) return "-";
        //            return v.ToString("0.###");
        //        }

        //        string GetCondition(TestConditionItem it)
        //        {
        //            if (it == null) return "";
        //            try
        //            {
        //                var typeStr = it.Type.ToString().Trim().ToUpperInvariant();
        //                var prop = TypeDescriptor.GetProperties(it)["SourceValue"];
        //                double src = 0;
        //                bool ok = prop != null && prop.GetValue(it) != null &&
        //                          double.TryParse(Convert.ToString(prop.GetValue(it)), out src);
        //                if (!ok) return "";
        //                if (typeStr == "IR") return $"{src:0.###}V";
        //                if (Math.Abs(src) >= 1.0) return $"{src:0.###}A";
        //                if (Math.Abs(src) >= 1e-3) return $"{src * 1e3:0.###}mA";
        //                if (Math.Abs(src) >= 1e-6) return $"{src * 1e6:0.###}uA";
        //                return $"{src:E3}A";
        //            }
        //            catch { return ""; }
        //        }
        ...
        //        bool fileExists = System.IO.File.Exists(logFile);
        //        using (var writer = new System.IO.StreamWriter(logFile, true, Encoding.UTF8))
        //        {
        //            if (!fileExists)
        //            {
        //                // 헤더1: No,BIN,NO,1,,2,,...
        //                var h1 = new StringBuilder("No,BIN,NO");
        //                for (int i = 0; i < items.Count; i++)
        //                    h1.Append($",{i + 1},"); // 인덱스, 측정값 컬럼
        //                writer.WriteLine(h1.ToString());

        //                // 헤더2: ,,Name,VR1,,VF3,,...
        //                var h2 = new StringBuilder(",,Name");
        //                for (int i = 0; i < items.Count; i++)
        //                    h2.Append($",{Csv(items[i]?.Name ?? "")},");
        //                writer.WriteLine(h2.ToString());

        //                // 헤더3: ,,ApplyValue,10mA,,150mA,,...
        //                var h3 = new StringBuilder(",,ApplyValue");
        //                for (int i = 0; i < items.Count; i++)
        //                    h3.Append($",{Csv(GetCondition(items[i]))},");
        //                writer.WriteLine(h3.ToString());
        //            }

        //            // 데이터행: No,BIN,Name,범위,측정값,...
        //            var row = new StringBuilder();
        //            row.Append(nextRowNo);
        //            row.Append($",{binNo}");
        //            row.Append($",{Csv(nameCell)}");

        //            for (int i = 0; i < items.Count; i++)
        //            {
        //                var itemName = items[i]?.Name ?? "";
        //                row.Append($",{Csv(GetRange(itemName))},{Csv(GetMeasured(itemName))}");
        //            }
        //            writer.WriteLine(row.ToString());
        //        }

        //        System.IO.File.WriteAllText(noFile, nextRowNo.ToString());
        //        return 0;

        //        string GetRecipeNameFallback(string waferId, string fallback)
        //        {
        //            try
        //            {
        //                var recipe = Equipment.Instance?.EquipmentRecipe?.CurrentRecipe as QMC.LCP_280.Process.Component.MeasurementRecipe;
        //                if (!string.IsNullOrWhiteSpace(recipe?.Name)) return recipe.Name;
        //                if (!string.IsNullOrWhiteSpace(recipe?.TestConditionSetPath))
        //                    return System.IO.Path.GetFileNameWithoutExtension(recipe.TestConditionSetPath);
        //            }
        //            catch { }
        //            return string.IsNullOrWhiteSpace(waferId) ? fallback : waferId;
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        //        Log.Write(UnitName, $"[AssignDataToMaterialObject] Exception: {ex.Message}");
        //        return -1;
        //    }
        //}
        ...
        //        var set = tester?.ConditionSet;
        //        var res = tester?.Result;
        //        var sheet = tester?.BinningSpecSheet;
        //        if (set == null || res == null || sheet == null) return -1;
        ...
        //        var bin = res.BinningResult;
        //        int binNo = bin?.BinNo ?? -1;
        //        string binLabel = string.IsNullOrWhiteSpace(bin?.BinLabel) ? "Test" : bin.BinLabel;

        //        string nameCell = GetRecipeNameFallback(waferID, binLabel);

        //        var items = set.Items?.ToList() ?? new List<TestConditionItem>();
        //        BinningSpec binSpec = sheet.Specs.FirstOrDefault(s => string.Equals(s.BinLabel, binLabel, StringComparison.OrdinalIgnoreCase));
        ...
        //        // 범위 문자열 (항상 "min~max" 또는 "- ~ -")
        //        string GetRange(string itemName)
        //        {
        //            if (string.IsNullOrWhiteSpace(itemName) || binSpec == null) return "- ~ -";
        //            if (!binSpec.Items.TryGetValue(itemName, out var range) || range == null) return "- ~ -";
        //            if (range.Ignore) return "- ~ -";
        ...
        //                var h1 = new StringBuilder("No,BIN,NO");
        //                for (int i = 0; i < items.Count; i++)
        //                    h1.Append($",{i + 1},"); // 인덱스 컬럼 + 측정값 컬럼(헤더명 비움)
        //                writer.WriteLine(h1.ToString());

        //                // 헤더2: ,,Name,VR1,,VF3,,Watt,,...
        //                var h2 = new StringBuilder(",,Name");
        //                for (int i = 0; i < items.Count; i++)
        //                    h2.Append($",{Csv(items[i]?.Name ?? "")},"); // 항목명 + 측정값 헤더(비움)
        //                writer.WriteLine(h2.ToString());

        //                // 헤더3: ,,ApplyValue,10mA,,150mA,,0.000E+000A,,...
        //                var h3 = new StringBuilder(",,ApplyValue");
        //                for (int i = 0; i < items.Count; i++)
        //                    h3.Append($",{Csv(GetCondition(items[i]))},"); // 조건 + 측정값 헤더(비움)
        //                writer.WriteLine(h3.ToString());
        //            }

        //            // 데이터행: No,BIN,Name,범위,측정값,범위,측정값,...  (측정값 위치가 기존 & 자리)
        //            var row = new StringBuilder();
        //            row.Append(nextRowNo);
        //            row.Append($",{binNo}");
        //            row.Append($",{Csv(nameCell)}");
        ...
        //            for (int i = 0; i < items.Count; i++)
        //            {
        //                var itName = items[i]?.Name ?? "";
        //                string rangeStr = GetRange(itName);
        //                string measStr = GetMeasured(itName);
        //                row.Append($",{Csv(rangeStr)},{Csv(measStr)}");
        //            }
        ...
        //        // 헬퍼
        //        string GetRecipeNameFallback(string waferId, string fallbackLabel)
        //        {
        //            try
        //            {
        //                var recipe = Equipment.Instance?.EquipmentRecipe?.CurrentRecipe as QMC.LCP_280.Process.Component.MeasurementRecipe;
        ...
        //            }
        //            catch { }
        //            return string.IsNullOrWhiteSpace(waferId) ? fallbackLabel : waferId;
        //        }
        //    }
        //    catch (Exception ex)
        //    {
