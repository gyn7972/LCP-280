    ...
    public class CASSpectrometer : BaseComponent, IDisposable
    {
        ...
        private bool InitializeDevice()
        {
            ...
            try
            {
                ...
                if (Config.UseHardwareTrigger)
                {
                    ...
                    //SetMeasurementParameter(CAS4DLL.mpidTriggerSource, 2);
                    ...
                    //add 2025-12-06
                    SetMeasurementParameter(CAS4DLL.mpidTriggerDelayTime, 5);// ms 
        ...
        private bool SendMeasureCommand()
        {
            try
            {
                ...
                // 검증 로그
                //double delay = CAS4DLL.casGetMeasurementParameter(deviceId, CAS4DLL.mpidTriggerDelayTime);
                //double acqTime = CAS4DLL.casGetMeasurementParameter(deviceId, CAS4DLL.mpidACQTime);
                //double sigLevel = CAS4DLL.casGetMeasurementParameter(deviceId, CAS4DLL.mpidSignalLevel);
                //Log.Write("CAS", "Trigger/Timing", $"Delay={delay}ms, ACQTime={acqTime}ms, SignalLevel={sigLevel}%");
