...
namespace QMC.Common.Keithley
{
    public class KeithleySourcemeterChannel
    {
        ...
        // VF3 앞에 VR이 없어서 주입형 VR을 실행했는지 여부 (결과 매핑 시 스킵을 위해 사용)
        private bool injectedVrForFirstVf3 = false;
        ...
        public bool RunContactRCommand()
        {
            try
            {
                ...
                foreach ( var v in this.commands)
                {
                    ...
                    {
                        dHiCurrent = (v.SourceValue / 1000000);
                    }
                    if(v.Action == CommandAction.MeasureContactRLow)
                    {
                        dLowCurrent = (v.SourceValue / 1000000);
                    }
                }
