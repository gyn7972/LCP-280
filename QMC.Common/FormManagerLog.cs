...
namespace QMC.Common
{
    ...
    public class FormManagerLog
    {
        ...
        public void RegisterLogForm(Type formType, string displayName, string description = null, int order = int.MaxValue)
        {
            FormManager.Instance.RegisterForm(MenuButtonType.Log, formType, displayName, description ?? displayName, order);
        }
        ...
        public void AutoRegisterUnitLogForms(System.Reflection.Assembly assemblyToSearch = null)
        {
            try
            {
                ...
                foreach (var (type, order) in formTypes.OrderBy(x => x.order).ThenBy(x => x.type.Name))
                {
                    ...
                    RegisterLogForm(type, unitName, $"{unitName} Log Viewer", order);
                }
            }
            catch (Exception ex)
            {
                var mb = new MessageBoxOk();
                mb.ShowDialog("Warning!", $"Unit Log 폼 자동 등록 중 오류: {ex.Message}");
            }
        }
        ...
        private string ExtractUnitNameFromType(Type type)
        {
            ...
            {
                result = typeName.Replace("Unit_Log", "");
            }
            else if (typeName.Contains("UnitLog"))
            {
                result = typeName.Replace("UnitLog", "");
            }
            else if (typeName.EndsWith("Log"))
            {
                result = typeName.Replace("Log", "");
            }
            else
            {
                result = typeName;
